...
```{r}
many.fits <- apply(merged.data[,4:10], MARGIN=2, function(x) lm(x ~ region1, data= merged.data))
```
...
* (4) Make the SNP positions, the row names of these data frames. (Hint: You will need to first save the column names of `merged.data` as an object). 
...
I don't have `sex` as a variable in this simulated data set, but we can test to see whether there is a significant interaction, but changing our `lm( )`. You will multiply your "SNP" by `sex` with an `*` before the `~`. 
